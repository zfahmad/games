#ifndef ITERATIVE_DEEPENING_DEPTH_FIRST_SEARCH_HPP
#define ITERATIVE_DEEPENING_DEPTH_FIRST_SEARCH_HPP
...
template <typename Game> void Solver<Game>::run_DFS() {
    num_expansions = 0;
    max_depth = 1;
    repeat = true;
    int depth = 0;
    ...
    while (repeat) {
        repeat = false;
        std::cout << "repeat with depth: " << max_depth << std::endl;
        value = DFS(depth);
        // max_depth *= 2;
        max_depth++;
    }
...
template <typename Game> std::int8_t Solver<Game>::DFS(int depth) {
    this->num_expansions++;
    Outcomes outcome;
    ...
    for (auto &ancestor : history) {
        ...
        if (current == ancestor) {
            // std::cout << current[Player::One] << ", " << current[Player::Two]
            //           << " | ";
            // std::cout << ancestor[Player::One] << ", " << ancestor[Player::Two]
            //           << std::endl;
            return 0;
        }
    }
    ...
    if (game.is_terminal(state)) {
        ...
        for (auto action : actions) {
            ...
            std::int8_t child_utility = DFS(depth + 1);
